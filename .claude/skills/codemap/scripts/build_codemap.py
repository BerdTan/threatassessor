...
def cmd_gen_mmd(args):
    import fnmatch
    import glob as glob_mod

    root = os.path.abspath(args.root)

    # ── 1. Resolve module files ──────────────────────────────────────────
    module_files = []
    for pattern in args.modules:
        matches = sorted(glob_mod.glob(os.path.join(root, pattern)))
        if not matches:
            print(f"gen-mmd: warning: no files matched {pattern!r}")
        module_files.extend(matches)
    module_files = sorted(set(module_files))
    if not module_files:
        raise SystemExit("gen-mmd: no modules matched any pattern")

    # ── 2. Parse each module ─────────────────────────────────────────────
    registry = {}  # rel_path → {stem, classes, raw_imports}
    for path in module_files:
        ...
        try:
            ...
            continue
        classes = [m.group(1) for line in text.splitlines()
                   for m in [re.match(r'^class (\w+)', line)] if m]
        # Match both absolute and relative imports; skip leading dots
        raw_imports = re.findall(r'^(?:from|import)\s+\.*([\w][\w.]*)', text, re.MULTILINE)
        registry[rel] = {"stem": stem, "classes": classes, "raw_imports": raw_imports}

    stems_by_name = {}  # basename stem → rel_path (last wins on collision)
    for rel, info in registry.items():
        stems_by_name[info["stem"]] = rel

    # ── 3. Infer cross-module edges from imports ─────────────────────────
    edges = set()
    for rel, info in registry.items():
        for imp in info["raw_imports"]:
            for part in reversed(imp.split(".")):
                if part in stems_by_name and stems_by_name[part] != rel:
                    edges.add((rel, stems_by_name[part]))
                    break

    # ── 4. Assign unique node IDs ────────────────────────────────────────
    used_ids = {}
    node_id = {}
    for rel, info in registry.items():
        base = re.sub(r'[^A-Za-z0-9]', '_', info["stem"]).upper()
        if base in used_ids:
            base = f"{base}_{len(used_ids)}"
        used_ids[base] = rel
        node_id[rel] = base

    # ── 5. Parse groups (NAME:PAT1,PAT2,... fnmatch on stem) ────────────
    groups = {}
    ungrouped = list(registry.keys())
    for g_spec in (args.group or []):
        name, _, patterns_str = g_spec.partition(":")
        patterns = [p.strip() for p in patterns_str.split(",")]
        matched = [r for r in ungrouped
                   if any(fnmatch.fnmatch(registry[r]["stem"], p) for p in patterns)]
        if matched:
            groups.setdefault(name, []).extend(matched)
            ungrouped = [r for r in ungrouped if r not in matched]
    if ungrouped:
        groups["Other"] = ungrouped

    # ── 6. Emit flowchart .mmd ───────────────────────────────────────────
    GROUP_STYLES = {
        "Brain":     ("brain", "fill:#1e293b,stroke:#475569,color:#e2e8f0"),
        "TACO":      ("taco",  "fill:#0f172a,stroke:#0ea5e9,color:#bae6fd"),
        "Workspace": ("ws",    "fill:#14532d,stroke:#16a34a,color:#dcfce7"),
        "API":       ("api",   "fill:#1e1b4b,stroke:#818cf8,color:#e0e7ff"),
        "Other":     ("other", "fill:#292524,stroke:#78716c,color:#d6d3d1"),
    }
    title = args.title or "Architecture"
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    lines = [
        "---",
        f"title: {title}",
        f"updated: {today}",
        'note: "Generated by build_codemap.py gen-mmd. Re-run after module changes."',
        "---",
        "flowchart TD",
    ]
    class_assigns = []
    for group_name, rels in groups.items():
        safe_gid = "SG_" + re.sub(r'[^A-Za-z0-9_]', '_', group_name)
        lines.append(f'    subgraph {safe_gid}["{group_name}"]')
        for rel in rels:
            info = registry[rel]
            nid = node_id[rel]
            cls_sample = "<br/>".join(info["classes"][:3])
            label = info["stem"]
            if cls_sample:
                label += f"<br/>{cls_sample}"
            lines.append(f'        {nid}["{label}"]')
            if group_name in GROUP_STYLES:
                class_assigns.append((nid, GROUP_STYLES[group_name][0]))
        lines.append("    end")
        lines.append("")

    for from_rel, to_rel in sorted(edges):
        lines.append(f"    {node_id[from_rel]} --> {node_id[to_rel]}")
    if edges:
        lines.append("")

    active_styles = {cls for _, cls in class_assigns}
    for gname, (cls, style_str) in GROUP_STYLES.items():
        if cls in active_styles:
            lines.append(f"    classDef {cls} {style_str}")
    for nid, cls in class_assigns:
    ...
    with open(out_path, "w") as f:
        f.write("\n".join(lines) + "\n")
    print(f"gen-mmd: {len(registry)} modules, {len(edges)} edges -> {args.output}")
...
def main():
    ...
    p_genmmd = sub.add_parser("gen-mmd", help="generate a Mermaid architecture diagram from module imports")
    p_genmmd.add_argument("--root", default=".", help="repo root (default: .)")
    p_genmmd.add_argument("--modules", nargs="+", required=True,
                          help="module paths or glob patterns relative to --root")
    p_genmmd.add_argument("--output", required=True, help="output .mmd file path")
    p_genmmd.add_argument("--title", default=None, help="diagram title for MMD frontmatter")
    p_genmmd.add_argument("--group", action="append", default=None,
                          help="NAME:PATTERN — group modules by fnmatch on stem; may repeat")
    p_genmmd.set_defaults(func=cmd_gen_mmd)
