...
**What was decided:**
`agent_framework.py` now retries LLM calls up to 2 times when `_parse_response()` returns `{}`. On retry, a prefix is prepended: "IMPORTANT: Your previous response was not valid JSON. You MUST respond with a single \`\`\`json code block…". All 5 critic prompts (Architect, Tester, Red Team, Purple Team, Blackhat) have a `RESPONSE FORMAT:` anchor added as the first line, before any artifacts, committing the model to a single JSON block before it sees the data. Root cause: Bedrock transient responses can return non-JSON content on degraded calls; previously `score=0 / rating=UNKNOWN` silently propagated.

**Reasoning:**
The zero-score on `22_generic_name_with_ai_nodes` was confirmed transient (re-run produced score=73). Prompt instructions at the bottom (~400 lines in) can be missed on degraded responses; anchoring at the top forces model commitment before context load. Two retries balance cost vs reliability — a third call is cheap insurance against a random Bedrock hiccup.

**Alternatives rejected:**
- Silent default score=50 on parse failure: masks real failures and inflates confidence.
- Single retry only: one bad response in a row is plausible on Bedrock; two consecutive is rare enough to surface as a real error.

### 2. Sequential vs partial_parallel mode — no meaningful difference on current corpus

**What was decided:**
Keep `partial_parallel` as the default mode. Comparison across 3 architectures (21_agentic, 05_legacy, 10_complex_enterprise) showed final confidence identical to 1 decimal place for 2/3 archs; the third improved 2.3pp in sequential only because Red Team moved between penalty bands (score 60→55). `cross_pct` (technique corroboration) was unchanged in all three — mode does not affect it.

**Reasoning:**
Sequential mode's theoretical benefit (Red Team reads Tester's gaps) only materialises when Tester surfaces subtle findings Red Team would otherwise miss. On the current corpus (architectures with visible high-severity gaps), Red Team finds absence-of-controls independently. The real lever for cross_pct is running more full_moe passes, not changing mode. Sequential adds ~30-40s wall-clock for no measurable gain.

**Alternatives rejected:**
- Switch to sequential as default: slower, same output, no evidence of quality improvement on this corpus.
- `auto` mode: complexity-adaptive but adds unpredictability; prefer explicit default.

### 3. Tester scoped to factual accuracy; Red Team scoped to per-control bypass depth

**What was decided:**
Tester is a pure accuracy auditor: owns MITRE M-ID→T-ID correctness, risk arithmetic consistency, and reporting completeness. Explicitly NOT coverage sufficiency (Purple Team) or bypass capability (Red Team). The "VERIFY COVERAGE" task section was replaced with "VERIFY REPORTING COMPLETENESS" — removes "3+ controls per critical path" and "high-risk categories need multiple controls" judgements.

Red Team is a per-control bypass analyst: for each deployed control on each attack path, reasons through specific bypass techniques (WAF encoding evasion, MFA AiTM/Evilginx, EDR LOLBins, IDS encrypted C2, etc.). Absent controls are noted in one sentence then analysis moves on. New `control_bypass_analysis` array added to output JSON schema (additive, does not break existing consumers). `exploit_mitigation_roadmap` and `path_assessments` preserved.

All surface labels updated in sync: dashboard.js (4 desc/role strings + file catalogue + settings panel), executive_dashboard_generator.py (Layer 2B/2C + validation table), improvement_summary_generator.py (critic label tuple).

**Reasoning:**
All five critics were converging on "no WAF / no auth / no segmentation" because Tester's coverage-sufficiency checks and Red Team's overall-difficulty scoring both pattern-match on control presence/absence. The fix is scope boundaries, not new critics. Five distinct questions: Architect = design opinion, Tester = factual accuracy, Red Team = bypass realism, Purple Team = coverage completeness, Blackhat = cross-path chaining.

**Alternatives rejected:**
- Renaming breakdown sub-fields (validation_checks etc.): dashboard.js renders them by key name; renaming breaks the score-chip row silently. Kept all four keys, changed what belongs in each.
- Adding a sixth "bypass specialist" critic: unnecessary — Red Team already owns this lane, it just wasn't using it.
...
**Tester (tester_critic.py):** Computes which detect-only techniques appear in this arch's attack paths; injects the list into the prompt. "VERIFY REPORTING COMPLETENESS" now explicitly says detect-only techniques should be `detect_only_gap` category with MEDIUM severity + recommend a detection control — not flagged as missing M-IDs. `detect_only_gap` added as valid category in the Tester JSON output schema.
...
**Promotion threshold = 3 archs** to avoid false positives from single-arch quirks (e.g. Architect hallucinating "0 components" on `10_complex_enterprise`). After 2 ER runs (61 signals, 28 `detect_only_gap`), promotion is blocked until a third arch's run adds confirmation.

**Reasoning:**
The long-term goal is closing the feedback loop: critics surface patterns → patterns bake into engine rules → engine produces better output → critics find fewer gaps. The `pending_review` status ensures a human confirms each promotion before it changes engine behavior — no autonomous rewriting of analysis rules. `engine_hints.json` is the handoff point; downstream consumption by ground_truth_generator is the next implementation step.

**Alternatives rejected:**
- Direct autonomous engine rewriting from critic output: too risky without human review; a single bad arch run could corrupt the engine.
- In-memory accumulation only: signals lost between sessions; file-based JSONL is simple and auditable.
- Promotion threshold = 1: too aggressive; a single-arch hallucination would promote immediately.
...
**What was decided:**
Created `tests/test_harness_event_broker.py` — full regression coverage for the EventBroker pipeline telemetry layer. 8 test classes, 48 tests, ~9s, no LLM calls or network access. All external dependencies mocked:
- `langfuse.Langfuse` — patched at `langfuse.Langfuse` (local import inside `_init_client`)
- `urllib.request.urlopen` — patched at stdlib level (local import inside `emit()`)
- `chatbot.config.settings.get_settings` — patched for settings override test

**Coverage per class:**
- `TestHarnessEvent` — to_dict round-trip, all 7 event types, default payload
- `TestEventBrokerInit` — policy loading, settings override disables broker, SiemSink registration
- `TestEventBrokerEmit` — fan-out, disabled guard, per-sink failure isolation, flush
- `TestBaseSinkFiltering` — all 4 preset names expand correctly (stage_trace, critic_trace, governance, aivss)
- `TestSiemSink` — JSONL write, aivss_gate→SiemEvent schema, governance translation, subscription filter, webhook
- `TestLangfuseSink` — credential resolution order (config > env > default), all 7 event-type→Langfuse-call mappings, flush, _lf=None guard
- `TestWebhookSink` — POST body shape, no-URL guard, failure isolation, subscription filter
- `TestMultiSinkIntegration` — SiemSink (real file write) + mock Langfuse + mock Webhook fan-out

**Key finding during test authoring:** `settings.event_broker.enabled` defaults to `False` in `AppSettings`, which silently disables the broker when the settings check fires inside `_load_sinks()`. Any test that exercises sink registration must mock settings to return `enabled=True`.

**`check-eventbroker` skill** added — runs regression suite + shows live broker config from `policies/agent_governance.yaml` + last 5 SIEM events from `logs/siem.jsonl`. Named arch mode shows per-architecture status.

**ED end-to-end verification path** documented in SKILL.md: analysis run → `tail logs/siem.jsonl` → Langfuse UI for traces (requires Langfuse running + keys in .env + langfuse.enabled: true in YAML).
...
**What was decided:**
Add `technique_support: Dict` field to `MoEResult` dataclass, computed deterministically after LLM synthesis (no extra API call). Regex-scans each critic's raw output JSON for T-IDs, deduplicates per critic, counts distinct critics per technique. Flags `mandatory: True` for Red-Team-only and Blackhat-only findings — signals most suppressed by anchoring bias. Stored in `07_moe_orchestrator.json`. Both `tatb-score.py` and `dashboard.js` use stored counts first, fall back to blob-regex for pre-Phase-4 reports. TTP-Accurate evidence pane shows mandatory badges for single-critic findings.

**Reasoning:** The blob regex scan overcounted (T-IDs appearing incidentally in prose inflated cross_pct). Stored deduped counts give true independent-agreement signal. Mandatory flag ensures Blackhat/Red-Team-only findings are never silently filtered as UNSURE.

---

### 2. Critic output tone — all five critics tightened to ONE sentence per field

**What was decided:**
Four rounds of tightening across all five MoE critics (Tester, Architect, Purple Team, Blackhat, Orchestrator) and SM. Pattern: description = ONE sentence stating the gap fact, stop; recommendation = ONE action, no sub-steps; verification_method = ONE concrete check (grep command or count); rationale = no citation dump.

Key rules added to each prompt:
- DELETE trigger words: "including", "but assessment", "This indicates", "Attacker can", "Adversary can" — everything from that point is consequence chain, not fact.
- ONE semicolon in recommendation = automatic fail.
- Tag/control-token categories → CRITICAL + `blocked=True` on single match.

SM redesign path (deterministic): `action` now truncates to first sentence; `rationale` replaced with generic 2-sentence structural note; `first_step` keyword extractor skips metric/score words.

**Reasoning:** LLM fills unconstrained free-text fields by quoting its input back (critic gap text, technique IDs, numbered lists). Explicit field-level length rules + GOOD/BAD examples are more reliable than "be concise" instructions.

---

### 3. SM AP closure enforcement on redesign path

**What was decided:**
`_build_action_plan()` was returning early via `_build_redesign_recommendations()` on `redesign_signal=True`, bypassing the AP closure loop. After `_build_redesign_recommendations()` returns, apply the same AP closure enforcement as the LLM path — for each CRITICAL AP not referenced in any item, append a targeted plan item.

**Result:** `05_legacy_flat_network` AP plan closure 0% → 67%, Plan-Actionable 77 → 88 Excellent.

---

### 4. Prompt injection guardrail — 9 categories + 3 evasion layers

**What was decided:**
Expanded `InhouseGovernanceAdapter.check_input()` in `chatbot/harness/governance.py`:

- `_normalise()` gains 3 new evasion layers: character-spacing collapse ("i g n o r e" → "ignore"), base64 decode (appends decoded content for scanning), typoglycemia map (ignroe→ignore, byp@ss→bypass).
- `_RE_INJECTION` replaced with `_INJECTION_PATTERNS` dict — 9 named categories each with a severity weight. New categories: system_override, developer_mode, dan_jailbreak, safety_bypass, prompt_extraction. Tag/control-token categories (LLM special tokens, zero-width chars) → CRITICAL on single match.
- Severity now category-based: any CRITICAL category → CRITICAL+blocked on 1 match (was: only path traversal / oversized labels). Any HIGH category → HIGH on first match (was: needed count ≥2).
- `injection_categories` dict added to `sig.exploitation` for SIEM audit trail.

**Reasoning:** Prior gate missed 6 attack categories and 3 evasion techniques that OpenRouter documents as their detection stack. Implemented locally — no API dependency, zero latency overhead, works on Bedrock.

**Tests:** 56 passing, 2 xfailed. 11 new fixture MMDs, 3 new test classes. `check-governance` skill added.

---

### 5. New skills: regen-reports, check-governance; corpus ER batch

**regen-reports:** Regenerates all 13 report MD/MMD files per architecture from existing JSON data (~2-5s/arch). Use after any generator code change. Modes: default (all layers), --md-only, --dashboard-only, --improvement-only, --all, --dry-run.

**check-governance:** Runs governance regression suite (56 tests, ~9s, no LLM). Optional live signal check for one or all corpus architectures from `governance_signals.json`.

**Corpus ER re-run:** Full_moe ER on all 26 architectures to bake in SM quality improvements, tone fixes, technique_support, and AP closure enforcement. 21 passed, 5 failed (API server restart required mid-run — 15_cdn, 17_multi_region, 19_blockchain, 22_generic_ai, 99_naked). Final corpus: avg=85.1, min=79 (03_aws_3tier), max=90 (22_generic_name_with_ai_nodes).

---

### 6. TATB corpus UI — lazy-load regression panel

**What was decided:**
The TATB tab previously fired Promise.all across all 26 archs (up to 52 parallel fetches) before the scorecard rendered. Now: 1 fetch on load (checks if current arch has labels), scorecard renders immediately, corpus regression panel loads on "▶ Load regression" button click.

---

## 2026-07-12 (Session 15, continued) — Critic Pipeline Upgrade + Report Quality + Skills

### 1. partial_parallel as default critic mode

**What was decided:**
`partial_parallel` replaces `sequential` as the default across all 5 entry points (settings.py, controller.py, stages.py, moe_orchestrator.py, streaming.py). In partial_parallel: Architect and Red Team run concurrently (Red Team is blind to Tester at LLM time); Tester reads Architect after; Purple Team and Blackhat always run sequentially with full context.

**Reasoning:**
In sequential mode Red Team anchors on Tester's framing — if Tester missed a surface, Red Team tends to miss it too. Partial_parallel gives Red Team an independent view of the attack surface, reducing anchoring bias while preserving Tester's cross-reference. Verified: `critic_mode: partial_parallel` stored in 07_moe_orchestrator.json after re-run.

**Alternatives rejected:**
- Keeping sequential as default: perpetuates anchoring. `auto` mode also resolved to sequential for all real architectures (21_agentic scores 147 vs threshold 60).

---

### 2. SM action plan — CISO/CIO audience + phasing + ADR context injection

**What was decided:**
Three changes to `scrum_master_critic.py`: (a) LLM prompt now names dual audience (CISO/CIO for deployment approval + engineer for implementation); rationale instruction extended to 2 sentences (attacker prevention + business consequence avoided); sequencing principle added (order by time-to-risk-reduction). (b) ADR-mandated control names injected into SM prompt from `ground_truth["architecture_decision_records"]` — same enforcement pattern as AP-IDs. (c) Redesign Tier A `first_step` now uses a control keyword from the action text instead of identical boilerplate; rationale truncation 120→300 chars; action plan sorted CRITICAL before HIGH.

**Result:** ADR alignment 50%→83% after fresh ER run. Plan-Actionable 95→98.

---

### 3. TATB scoring — ADR alignment field bug + rationale check

**What was decided:**
Two fixes: (a) dashboard.js read `ctrl.name` instead of `ctrl.control` for ADR hop controls — `adrControls` was always size 0, alignment always 50% neutral. (b) ADR alignment check extended to include `rationale` field alongside `first_step + action` in both tatb-score.py and dashboard.js.

**Result:** ADR alignment immediately corrects to measured % from existing SM data without a new ER run.

---

### 4. ATLAS/AML rendering in report files

**What was decided:**
`threat_report.py`: (a) 02_technical_report.md now splits techniques into MITRE ATT&CK and ATLAS sections — AML.* IDs route to AtlasHelper for name + description instead of bare ID strings. (b) 09_threat_model.md adds an "AI-specific threats (MITRE ATLAS)" paragraph per AP when ATLAS techniques are present, explaining that standard controls don't address model/prompt/inference attack vectors. Gate is natural — empty list for non-AI architectures.

`improvement_summary_generator.py`: `_generate_gaps` and `_generate_strengths` now read from `expert_validations.consensus_recommendations.critical[]` (MoE format) instead of legacy `unified_roadmap` — was showing "No critical gaps identified" for all MoE runs.

---

### 5. New skills: rerun-corpus, regen-reports, codemap

**What was decided:**
Three skills added:
- `rerun-corpus` — corpus staleness audit + selective re-run of deterministic engine. ENGINE_FIXES table tracks which arch types need re-running after score-affecting engine changes.
- `regen-reports` — regenerate all 13 report MD/MMD files from existing JSON data (~2-5s/arch, 12.6s for full 26-arch corpus). Use after any generator code change instead of re-running analysis or ER.
- `codemap` — structural index of codebase (356 files, 141k lines). Collapses large dirs.

---

### 6. Dashboard + UI fixes

**Rerun mode dropdown:** added `partial_parallel`; fixed invisible text (background/color contrast bug on light themes); "Last ER ran as:" now shows stored mode in primary color.
**run-er skill:** all 4 modes supported; `--full` no longer hardcodes sequential; SKILL.md rewritten with decision table.
**TATB rubric doc** (`_tatbShowRubricDoc`): fully rewritten to match current scorer — correct weights, real signals per rubric, CONFIRMED/PLAUSIBLE/FAILED classes, hygiene correctly marked as display-only.
**`_CTRL_SYNONYMS`** in dashboard.js: synced with tatb-score.py (15 new entries).
**`docs/TATB_RUBRIC.md`**: rewritten to match current scorer; linked from README.md "Benchmark methodology" section.
