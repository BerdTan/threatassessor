...
"""
    ...
    # --- Blackhat section (appended when 06b_blackhat_critique.json exists) ---
...
## Blackhat Perspective

> Layer 2D: Cross-path chain analysis — findings NOT covered by Architect, Tester, or Red Team.

**Cross-chain score:** {bh_score}/100 ({bh_rating}) ← inverted; higher = easier cross-path chain

**Stealth score:** {stealth} Defense Evasion technique(s) in chains{(' (' + ', '.join(stealthy_techs) + ')') if stealthy_techs else ''}

"""
            if chains:
                bh_section += "**Chained exploit paths:**\n"
                for c in chains[:5]:
                    bh_section += f"- {c}\n"
                bh_section += "\n"

            if gaps:
                bh_section += "**Cross-path mitigation gaps (unique to chained view):**\n"
                for g in gaps[:5]:
                    bh_section += f"- {g}\n"
                bh_section += "\n"

            if new_findings:
                bh_section += "**New findings not raised by Red Team:**\n"
