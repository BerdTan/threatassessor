    ...
    for fname in ("07_moe_orchestrator.json", "07_orchestrator_report.json"):
        ...
        if p.exists():
            try:
                ...
                pass
    bh_adjustment = moe_orch.get("confidence", {}).get("adjustments", {}).get("blackhat", None)
...
"""
    ...
    # --- Layer 2E: Blackhat section (appended when 06c_blackhat_critique.json exists) ---
...
## Layer 2E: Blackhat Cross-Path Analysis
...
"""
            ...
            bh_section += (
                ...
                "[`threatmodel_adr.mmd`](threatmodel_adr.mmd) (ThreatModel+ADR diagram), "
                "and [`after_bh.mmd`](after_bh.mmd) (hardened architecture + BH chain overlay). "
                "When Blackhat ran, [`threatmodel_adr_bh.mmd`](threatmodel_adr_bh.mmd) shows "
                "the BH chain subgraphs appended to the ThreatModel+ADR view._\n"
            )

            # --- Enrichment: connect BH findings to improvement tiers ---
            lrp = breakdown.get("least_resistance_paths", [])

            tier_lines = []
            if gaps:
                gap_names = [g if isinstance(g, str) else str(g) for g in gaps[:3]]
                gap_str = ", ".join(f"_{n}_" for n in gap_names)
                tier_lines.append(
                    f"The **Quick Win** tier should include cross-path gap controls "
                    f"({gap_str}). "
                    f"These gaps represent **new attack paths not covered by per-path controls** — "
                    f"they only become exploitable when an attacker pivots across paths."
                )
            if lrp:
                tier_lines.append(
                    "The **Recommended** tier should additionally address the least-resistance "
                    "pivot chains identified below, which represent the most likely attacker routes."
                )
            tier_lines.append(
                "The diagram [`after_bh.mmd`](after_bh.mmd) shows these chain controls "
                "overlaid on the hardened architecture."
            )

            bh_section += "\n**What this means for your improvement tiers**\n\n"
            bh_section += "  ".join(tier_lines) + "\n\n"

            # --- Enrichment: New attack paths table (or bullet list fallback) ---
            bh_section += "**New attack paths identified by Blackhat (beyond per-path analysis)**\n\n"
            if lrp:
                bh_section += "| Chain | Criticality | Pivot Node | Action |\n"
                bh_section += "|-------|-------------|------------|--------|\n"
                for entry in lrp:
                    if isinstance(entry, dict):
                        chain_nodes = entry.get("chain", [])
                        chain_str = " → ".join(str(n) for n in chain_nodes) if chain_nodes else "N/A"
                        criticality = entry.get("chain_criticality", "UNKNOWN")
                        pivot = entry.get("pivot", "N/A")
                        action = "Add cross-path control at pivot node"
                        bh_section += f"| {chain_str} | {criticality} | `{pivot}` | {action} |\n"
                    else:
                        # Fallback: string entry
                        bh_section += f"| {entry} | UNKNOWN | N/A | Review manually |\n"
                bh_section += "\n"
            elif chains:
                for c in chains:
                    bh_section += f"- {c}\n"
                ...
                bh_section += "_No new cross-path attack paths identified._\n\n"

            content += bh_section
        except Exception as exc:
            logger.warning(f"Blackhat section skipped in 08: {exc}")

    # --- ScrumMaster section (appended when 08_scrum_master.json exists) ----
    sm_path = report_path / "08_scrum_master.json"
    if sm_path.exists():
        try:
            with open(sm_path) as f:
                sm = json.load(f)

            sm_conf0  = sm.get("initial_confidence", 0)
            sm_confF  = sm.get("final_confidence", 0)
            sm_delta  = sm.get("confidence_delta", 0)
            sm_iter   = sm.get("iterations_run", 0)
            sm_traj   = sm.get("confidence_trajectory", [sm_conf0])
            sm_trigg  = sm.get("critics_retriggered", [])
            sm_redesign = sm.get("redesign_signal", False)
            sm_note   = sm.get("synthesis_note", "")
            sm_plan   = sm.get("action_plan", [])
            sm_imps   = sm.get("impediments_found", [])
            sm_bf     = sm.get("baseline_feedback") or {}

            harmony_label = (
                "🔴 Redesign recommended" if sm_redesign else
                "✅ Harmony reached (≥90%)" if sm_confF >= 90 else
                "◑ Near-harmony"
            )
...
## ScrumMaster — Harmony Synthesis

> ScrumMaster reads all critic findings, identifies what's blocking a higher score,
> and works to resolve it through targeted critic re-triggering.

**Harmony state:** {harmony_label}
**Confidence trajectory:** {traj_str}
"""
            if sm_iter:
                sm_section += f"**Iterations run:** {sm_iter}  \n"
            if sm_trigg:
                sm_section += f"**Critics re-triggered:** {', '.join(sm_trigg)}  \n"
            if sm_delta != 0:
                sign = "+" if sm_delta > 0 else ""
                sm_section += f"**Confidence gain:** {sign}{sm_delta:.1f}%  \n"

            n_imps = len(sm_imps)
            n_unresolvable = sum(1 for i in sm_imps if not i.get("resolvable", True))
            if n_imps:
                sm_section += f"**Impediments found:** {n_imps} ({n_unresolvable} structurally unresolvable)  \n"
            ...
            if sm_redesign:
                sm_section += (
                    "\n> ⚠ **Architectural Redesign Recommended.** Incremental controls alone "
                    "cannot close the identified gaps. The ScrumMaster has flagged structural "
                    "changes as necessary.\n"
                )

            if sm_plan:
                sm_section += "\n### ScrumMaster Priority Actions\n\n"
                sm_section += "_These items have been merged into the Improvement Tiers above and `03_action_plan.md`._\n\n"
                prio_sym = {"critical": "🔴", "high": "🟠", "medium": "🟡", "low": "⚪"}
                for i, item in enumerate(sm_plan, 1):
                    sym = prio_sym.get(item.get("priority", "medium").lower(), "•")
                    sm_section += f"{i}. {sym} **{item.get('action', '')}**"
                    if item.get("effort"):
                        sm_section += f" _{item['effort']}_"
                    sm_section += "\n"
                    if item.get("rationale"):
                        sm_section += f"   > {item['rationale']}\n"

            weak_controls = sm_bf.get("weak_controls", [])
            pattern_gaps  = sm_bf.get("pattern_gaps", [])
            gt_gaps       = sm_bf.get("ground_truth_gaps", [])
            if weak_controls or pattern_gaps or gt_gaps:
                sm_section += "\n### Engine Improvement Notes\n\n"
                sm_section += "_Structured feedback for the next analysis pass — not immediate action items._\n\n"
                for c in weak_controls[:5]:
                    sm_section += f"- ⚠ Weak control (too generic): {c}\n"
                for g in pattern_gaps[:5]:
                    sm_section += f"- 🔍 Pattern gap: {g}\n"
                for g in gt_gaps[:3]:
                    sm_section += f"- 📊 Baseline gap: {g}\n"

            content += sm_section
        except Exception as exc:
            logger.warning(f"ScrumMaster section skipped in 08: {exc}")
