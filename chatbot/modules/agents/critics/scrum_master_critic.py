...
class ScrumMasterCritic:
    ...
    def _build_action_plan(
        ...
        prompt = (
            ...
            f"{recs_text}\n\n"
            "Return the top 5 as a JSON array. TONE: every field must be short and direct — a practitioner reads this to act, not to understand. No citations, no critic names, no gap IDs in output fields.\n\n"
            "For each item:\n"
            "- 'action': ONE sentence — what to build/configure/deploy. Name the specific control and node. "
            "Do NOT quote critic gap text. Do NOT start with '[Immediate]' or '[Structural]'. "
            "If CRITICAL AP, start with AP-ID: e.g. 'AP-1: Deploy MFA on WebUI login endpoint.'\n"
            "  GOOD: 'Deploy File Integrity Monitoring on WebServer and FileStorage nodes.'\n"
            "  BAD:  '[Immediate] T1018 and T1083 have zero mitigations — Attacker can enumerate...'\n"
            "- 'rationale': TWO short sentences max — (1) what attack this stops, (2) what business harm is avoided. "
            "No critic names, no gap citations, no technique IDs unless essential.\n"
            "  GOOD: 'Blocks reconnaissance that enables lateral movement. Prevents undetected data mapping before exfiltration.'\n"
            "  BAD:  'Actionable now. Purple Team gap lens A/CRITICAL: T1018 on WebServer (AP-2, AP-3)...'\n"
            "- 'confidence_gain': estimated % recovery (from conf_gain input)\n"
            ...
            "  GOOD: 'Install Wazuh FIM agent on WebServer; configure rules for /etc, /var/www, /home.'\n"
            "  BAD:  'Add this control to the relevant ADR and assign an owner.'\n\n"
